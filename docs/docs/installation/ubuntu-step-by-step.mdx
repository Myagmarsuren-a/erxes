...
mongo
```
If you have any trouble to connect mongo shell check the status with following command
```bash
...
● mongod.service - MongoDB Database Server
     ...
     Active: failed (Result: exit-code) since Wed 2021-02-03 15:59:18 +08; 9s ago
       Docs: https://docs.mongodb.org/manual
    Process: 51201 ExecStart=/usr/bin/mongod --config /etc/mongod.conf (code=exited, status=14)
   Main PID: 51201 (code=exited, status=14)

2nd month 03 15:59:18 erxes systemd[1]: Started MongoDB Database Server.
2nd month 03 15:59:18 erxes systemd[1]: mongod.service: Main process exited, code=exited, status=14>
2nd month 03 15:59:18 erxes systemd[1]: mongod.service: Failed with result 'exit-code'.
```
Please change permissions on following directories with commands shown below
```bash
...
```
After this you should able to connect mongo shell without problem.
...
server {
	...
	# erxes dashboard front is running on 4200 port.
	...
	# erxes dashboard api is running on 4300 port.
